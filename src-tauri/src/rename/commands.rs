...
fn get_default_templates() -> Vec<TemplateConfig> {
    vec![
        ...
        TemplateConfig {
            id: uuid::Uuid::new_v4().to_string(),
            name: "{Date:YYYYMMDD}-{Input:topic}-V{Num:1}.{Ext}".to_string(),
            pattern: "{Date:YYYYMMDD}-{Input:topic}-V{Num:1}.{Ext}".to_string(),
            created_at: chrono::Local::now().to_rfc3339(),
            updated_at: chrono::Local::now().to_rfc3339(),
        },
        TemplateConfig {
            id: uuid::Uuid::new_v4().to_string(),
            name: "{Num:01}-{Input:name}.{Ext}".to_string(),
            pattern: "{Num:01}-{Input:name}.{Ext}".to_string(),
            created_at: chrono::Local::now().to_rfc3339(),
            updated_at: chrono::Local::now().to_rfc3339(),
        },
        TemplateConfig {
